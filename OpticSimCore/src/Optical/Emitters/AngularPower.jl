...
Emitters.apply(::Lambertian, ::Transform, power::Real, ::OpticSimCore.Ray{<:Real,3}) = power
...
function Emitters.apply(d::Cosine, tr::Transform, power::Real, ray::OpticSimCore.Ray{<:Real,3})
    cosanglebetween = dot(OpticSimCore.direction(ray), forward(tr))
...
function Emitters.apply(d::Gaussian, tr::Transform, power::Real, ray::OpticSimCore.Ray{<:Real,3})
    l = dot(OpticSimCore.direction(ray), right(tr))
    m = dot(OpticSimCore.direction(ray), up(tr))

...
"""
abstract type AbstractSource{T} <: OpticSimCore.OpticalRayGenerator{T} end
...
function Emitters.generate(s::Source{T}, state::SourceGenerationState{T} = SourceGenerationState(-1, -1, zeros(Vec3{T}))) where {T<:Real}
    ...
    ray = OpticSimCore.Ray(o, d)
    power, wavelength = generate(s.spectrum)
    
    power = apply(s.power_distribution, s.transform, power, ray)

    # return (OpticSimCore.Ray(o, d), SourceGenerationState(state.n+1, origin_index, o))
    return (OpticSimCore.OpticalRay(ray, power, wavelength; sourcenum=s.sourcenum), SourceGenerationState(state.n+1, origin_index, o))
end

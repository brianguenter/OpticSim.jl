...
```julia
insidematerialid(i::OpticalInterface{T}) -> OpticSimCore.GlassCat.AbstractGlass
outsidematerialid(i::OpticalInterface{T}) -> OpticSimCore.GlassCat.AbstractGlass
...
insidematerialid(::NullInterface{T}) where {T<:Real} = glassid(OpticSimCore.GlassCat.Air)
outsidematerialid(::NullInterface{T}) where {T<:Real} = glassid(OpticSimCore.GlassCat.Air)
...
struct ParaxialInterface{T} <: OpticalInterface{T}
    ...
    function ParaxialInterface(focallength::T, centroid::SVector{3,T}, outsidematerial::Y) where {Y<:OpticSimCore.GlassCat.AbstractGlass,T<:Real}
...
struct FresnelInterface{T} <: OpticalInterface{T}
    ...
    function FresnelInterface{T}(insidematerial::Z, outsidematerial::Y; reflectance::T = zero(T), transmission::T = one(T), interfacemode = ReflectOrTransmit) where {Z<:OpticSimCore.GlassCat.AbstractGlass,Y<:OpticSimCore.GlassCat.AbstractGlass,T<:Real}
...
transmissiveinterface(::Type{T}, insidematerial::X, outsidematerial::Y) where {T<:Real,X<:OpticSimCore.GlassCat.AbstractGlass,Y<:OpticSimCore.GlassCat.AbstractGlass} = FresnelInterface{T}(insidematerial, outsidematerial, reflectance = zero(T), transmission = one(T))
reflectiveinterface(::Type{T}, insidematerial::X, outsidematerial::Y) where {T<:Real,X<:OpticSimCore.GlassCat.AbstractGlass,Y<:OpticSimCore.GlassCat.AbstractGlass} = FresnelInterface{T}(insidematerial, outsidematerial, reflectance = one(T), transmission = zero(T))
opaqueinterface(::Type{T} = Float64) where {T<:Real} = FresnelInterface{T}(OpticSimCore.GlassCat.Air, OpticSimCore.GlassCat.Air, reflectance = zero(T), transmission = zero(T))

...
using .OpticSimCore.GlassCat: AbstractGlass, TEMP_REF, PRESSURE_REF, Glass, Air
...
CSGOpticalSystem(
    ...
    temperature = OpticSimCore.GlassCat.TEMP_REF,
    pressure = OpticSimCore.GlassCat.PRESSURE_REF
)
```
"""
...
struct AxisymmetricOpticalSystem{T,C<:CSGOpticalSystem{T}} <: AbstractOpticalSystem{T}
    ...
    function AxisymmetricOpticalSystem{T}(
        ...
        for i in 2:nrow(prescription)-1
            ...
            elseif surface_type == "Aspheric"
                ...
                error(
                    ...
                    "please create an issue at https://github.com/microsoft/OpticSimCore.jl/issues/new."
                )
            end
        ...
        system = CSGOpticalSystem(
            OpticSimCore.LensAssembly(elements...), det, detectorpixelsx, detectorpixelsy, D; temperature, pressure
        )
...
function traceMT(
    ...
    det = OpticSimCore.detectorimage(copies[1][1])
    # sum all the copies
    @inbounds for i in 2:N
        OpticSimCore.sum!(det, OpticSimCore.detectorimage(copies[i][1]))
    end

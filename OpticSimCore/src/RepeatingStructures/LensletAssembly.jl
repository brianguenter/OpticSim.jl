...
struct Display{T} <: OpticSimCore.Surface{T}
    surface::OpticSimCore.Rectangle{T}
    pixellattice::LatticeBasis{2,T}
    pixelpitch::Tuple{T,T} # x,y pitch of pixels
    xresolution::T
    yresolution::T
    transform::OpticSimCore.Geometry.Transform{T}

    function Display(xres::Int64, yres::Int64, xpitch::Unitful.Length, ypitch::Unitful.Length, transform::OpticSimCore.Geometry.Transform{T}) where {T <: Real}
        ...
        surface = OpticSimCore.Rectangle(size[1] / T(2), size[2] / T(2), SVector(T(0), T(0), T(1)), SVector(T(0), T(0), T(0))) # surface normal is +z axis
...
struct LensletAssembly{T}
    lens::OpticSimCore.ParaxialLens{T}
    transform::OpticSimCore.Geometry.Transform{T}
    display::Display{T}
    worldtodisplay::OpticSimCore.Geometry.Transform{T}

    LensletAssembly(lens::OpticSimCore.ParaxialLens{T}, transform::OpticSimCore.Geometry.Transform{T}, display::Display{T}) where {T} = new{T}(lens, transform, display, display.transform * transform)
end
...
function beamenergy(assy::LensletAssembly{T}, displaypoint::AbstractVector{T}, pupilpoints::SMatrix{3,N,T}) where {N,T <: Real}
    ...
    if isempty(intsct)
        ...
        return OpticSimCore.area(beamintsct) / OpticSimCore.area(beamlens), centroid
    end
end
...
function activebox(lenslet::LensletAssembly{T}, worldpoly::SVector{N,SVector{3,T}}) where {T <: Real,N}
    ...
    for point in worldpoly
        ...
        for lenspoint in OpticSimCore.vertices(lens)
            ray = Ray(locpoint, lenspoint - locpoint)
            refractedray, _, _ = processintersection(interface(lens), lenspoint, ray, T(OpticSimCore.GlassCat.TEMP_REF), T(OpticSimCore.GlassCat.PRESSURE_REF))

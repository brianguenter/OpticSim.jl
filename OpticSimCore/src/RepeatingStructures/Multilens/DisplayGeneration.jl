...
using OpticSimCore.Geometry:Transform,world2local
using OpticSimCore:plane_from_points,surfaceintersection,closestintersection,Ray,Plane,ConvexPolygon,Sphere 
using OpticSimCore
using OpticSimCore.Repeat:tilevertices,HexBasis1,tilesinside
...
function project(point::AbstractVector{T},projectionvector::AbstractVector{T},surface::Union{OpticSimCore.LeafNode{T,N}, S}) where {T,N,S<:ParametricSurface{T,N}}
    ...
    if pointintsct === nothing #point didn't project to a point on the surface
        ...
        return OpticSimCore.point(pointintsct)
    end
end
export project
...
function project(vertices::R, projectionvector::AbstractVector{T}, surface::Union{OpticSimCore.LeafNode{T,N}, S}) where {T,N,S<:ParametricSurface{T,N},R<:AbstractMatrix{T}}
...
function eyeboxbounds(eyebox::OpticSimCore.Plane,eyerelief, dir::AbstractVector, radius,fovθ,fovϕ) 
...
function spherepolygon(vertices,projectiondirection,sph::OpticSimCore.LeafNode)
...
function spherepolygons(eyebox::Plane{T,N},eyerelief,sphereradius,dir,fovθ,fovϕ,lattice)::Tuple{Vector{ConvexPolygon{6,T}},Vector{Tuple{Int64,Int64}}} where{T,N}
    ...
    sph = OpticSimCore.LeafNode(Sphere(ustrip(mm,sphereradius)),Geometry.translation(0.0,0.0,sphereoriginoffset)) #Sphere objects are always centered at the origin so have to make 

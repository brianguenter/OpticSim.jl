...
function localframe(a::ParaxialLens)
    if OpticSimCore.shape(a) isa ConvexPolygon
        return OpticSimCore.localframe(OpticSimCore.shape(a))
    else
        return nothing
    end
end
...
function display_plane(lens) 
    center_point = centroid(lens) + -OpticSimCore.normal(lens)* OpticSimCore.focallength(lens)
    pln = Plane(OpticSimCore.normal(lens), center_point, vishalfsizeu = .5, vishalfsizev = .5,interface = opaqueinterface())
...
julia> hex12RGB()
ClusterWithProperties{12, 2, Float64, OpticSimCore.Repeat.RGBCluster}(LatticeCluster{12, 2, Float64, 
...
function setup_coordinate_frames()
    ...
    corneavertex = OpticSimCore.Data.cornea_to_eyecenter()
    eyeboxtransform = eyeballframe*OpticSimCore.translation(0.0,0.0,ustrip(mm,corneavertex))  #unfortunately can't use unitful values in transforms because the rotation and translation components would have different types which is not allowed in a Matrix.

...
function ModelEye(assembly::LensAssembly{T}; nsamples::Int = 17, pupil_radius::T = 3.0, detpixels::Int = 1000, transform::Transform{T} = identitytransform(T), detdatatype::Type{D} = Float32) where {T<:Real,D<:Real}
    cornea_front = AcceleratedParametricSurface(ZernikeSurface(6.9, radius = -7.8, conic = -0.5), nsamples, interface = FresnelInterface{T}(OpticSimCore.GlassCat.EYE.CORNEA, OpticSimCore.GlassCat.Air))
    cornea_rear = Plane(SVector{3,T}(0, 0, -1), SVector{3,T}(0, 0, -3.2))
    cornea = (cornea_front ∩ cornea_rear)(transform)

    anterior_chamber_front = AcceleratedParametricSurface(ZernikeSurface(6.0, radius = -6.7, conic = -0.3), nsamples, interface = FresnelInterface{T}(OpticSimCore.GlassCat.EYE.AQUEOUS, OpticSimCore.GlassCat.EYE.CORNEA))
    anterior_chamber_rear = Plane(SVector{3,T}(0, 0, -1), SVector(0.0, 0.0, -3.2), vishalfsizeu = 6.0, vishalfsizev = 6.0, interface = FresnelInterface{T}(OpticSimCore.GlassCat.EYE.AQUEOUS, OpticSimCore.GlassCat.EYE.VITREOUS))
    ...
    lens_front = AcceleratedParametricSurface(ZernikeSurface(5.1, radius = -10.0), nsamples, interface = FresnelInterface{T}(OpticSimCore.GlassCat.EYE.LENS, OpticSimCore.GlassCat.EYE.VITREOUS))
    lens_rear = AcceleratedParametricSurface(ZernikeSurface(5.1, radius = 6.0, conic = -3.25), nsamples, interface = FresnelInterface{T}(OpticSimCore.GlassCat.EYE.LENS, OpticSimCore.GlassCat.EYE.VITREOUS))
    lens_barrel = Cylinder(5.0, 5.0, interface = FresnelInterface{T}(OpticSimCore.GlassCat.EYE.LENS, OpticSimCore.GlassCat.EYE.VITREOUS))
    ...
    new_assembly = OpticSimCore.LensAssembly(assembly, cornea, anterior_chamber, pupil, vitreous_chamber, lens)
    retina = SphericalCap(
        11.0,
        asin(10 / 11),
...
function ArizonaEye(::Type{T} = Float64; accommodation::T = 0.0) where {T<:Real}
    ...
    return DataFrame(
        ...
        Material = [OpticSimCore.GlassCat.Air, OpticSimCore.GlassCat.modelglass(1.377, 57.1, 0.0), OpticSimCore.GlassCat.modelglass(1.337, 61.3, 0.0), OpticSimCore.GlassCat.modelglass(1.42 + 0.0026 * accommodation - 0.00022 * accommodation^2, 51.9, 0.0), OpticSimCore.GlassCat.modelglass(1.336, 61.1, 0.0), missing],

...
transmissivethingrating(period, orders) = ThinGratingInterface(SVector(0.0, 1.0, 0.0), period, AGFFileReader.Air, AGFFileReader.Air, minorder=-orders, maxorder=orders, reflectance=[0.0 for _ in (-orders):orders], transmission=[0.2 for _ in (-orders):orders])
reflectivethingrating(period, orders) = ThinGratingInterface(SVector(0.0, 1.0, 0.0), period, AGFFileReader.Air, AGFFileReader.Air, minorder=-orders, maxorder=orders, transmission=[0.0 for _ in (-orders):orders], reflectance=[0.2 for _ in (-orders):orders])

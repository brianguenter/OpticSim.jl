...
@testitem "Lenses" setup = [TestConstants] begin
    using Random
    using LinearAlgebra
    ...
    @testset "Refraction" begin
        ...
        for i in 1:test_n
            nₛ = randunit()
            rᵢ = randunit()
            rₜ = OpticSim.refractedray(ηᵢ, ηₜ, nₛ, rᵢ)
            if !(rₜ === nothing)
    ...
    @testset "Reflection" begin
        ...
        for i in 1:test_n
            ...
            if abs(r ⋅ nₛ) < 0.9999 # if r and n are exactly aligned then their sum will be zero, not a vector pointing in the direction of the normal
                reflected = OpticSim.reflectedray(nₛ, r)
    ...
    @testset "Paraxial" begin
        ...
        intsct = OpticSim.halfspaceintersection(surfaceintersection(l, r))

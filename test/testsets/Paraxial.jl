...
@testitem "ParaxialLens" begin
    ...
    function rayintersection(lens, incomingray)
        ...
        refrac, _, _ = OpticSim.processintersection(OpticSim.interface(lens), OpticSim.point(intsct), OpticSim.normal(lens), OpticalRay(incomingray, 1.0, 0.55), OpticSim.TEMP_REF, OpticSim.PRESSURE_REF, false)
        return refrac
    end
    ...
    @testset "Reversibility of rays for paraxial lenses" begin
        ...
        intsctpt = point(intsct)
        # compute the refracted ray and project this backwards to its intersection with the optical axis. This should be the same position as returned by virtualpoint()
        refrac1, _, _ = OpticSim.processintersection(OpticSim.interface(lens), OpticSim.point(intsct), OpticSim.normal(lens), OpticalRay(r1, 1.0, 0.55), OpticSim.TEMP_REF, OpticSim.PRESSURE_REF, false)
        r2 = Ray(-displaypoint, -direction)

        intsct = OpticSim.surfaceintersection(lens, r2)
        intsctpt = point(intsct)
        # compute the refracted ray and project this backwards to its intersection with the optical axis. This should be the same position as returned by virtualpoint()
        refrac2, _, _ = OpticSim.processintersection(OpticSim.interface(lens), OpticSim.point(intsct), OpticSim.normal(lens), OpticalRay(r2, 1.0, 0.55), OpticSim.TEMP_REF, OpticSim.PRESSURE_REF, false)
